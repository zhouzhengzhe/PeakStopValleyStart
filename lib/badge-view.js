...
export function panelFor(state, nowMs, labels = {}) {
  ...
  return {
    ...
    badge: text.live,
    rows: [
      // The one row that carries a dot: the tariff is the fact everything else
      // follows from, and the dot is the same colour as the character's.
      { label: text.phase, value: `${phaseChinese} (${phaseEnglish})`, tone: 'accent', dot: true },
      {
        label: text.decision,
      ...
      { label: text.next, value: nextValue, tone: 'normal', dot: false },
      {
        label: text.release,
        value:
...
export const BADGE_ACCENTS = Object.freeze({
  ...
  armed: Object.freeze({ shell: '#ff9500', core: '#ffe066', glow: 28 }),
  held: Object.freeze({ shell: '#007aff', core: '#70c2ff', glow: 30 }),
