...
/**
 * The colour each character pose is tagged with.
 *
 * One accent drives the status dot, the bubble's edge and the toolbar's emphasis,
 * so the three always agree about what is happening. They are theme tokens, not
 * literals with a hope attached: an unknown custom property resolves to nothing,
 * and a badge that quietly falls back to black on a dark theme is worse than one
 * that was never coloured.
 *
 * The mapping follows what the state *means* to the person paying the bill rather
 * than the brake's internal vocabulary: off-peak is the good, cheap state; the
 * brace is a warning; a hold is the plugin working, not a fault, so it is branded
 * rather than alarming; and a live override is the one genuinely costly state, so
 * it is the only red.
 */
export const BADGE_ACCENTS = Object.freeze({
  idle: 'var(--dsw-alias-label-secondary, #868e96)',
  armed: 'var(--dsw-alias-state-warn-primary, #f08c00)',
  held: 'var(--dsw-alias-brand-primary, #2563eb)',
  released: 'var(--dsw-alias-state-success-primary, #2f9e44)',
  override: 'var(--dsw-alias-state-error-primary, #e03131)',
});

/**
 * The accent for a published state.
 *
 * Derived from the same pose decision the character uses, so the dot, the bubble
 * and the character can never tell three different stories about one instant.
 *
 * @param {object|undefined} state - the published hold state.
 ...
 * @returns {{action: string, label: string, enabled: boolean, hint: string, tone: string}[]} the toolbar.
 */
export function toolbarFor(state, labels = {}) {
  ...
  return [
    { action: 'status', label: text.status, enabled: true, hint: '', tone: 'ghost' },
    {
      action: 'now',
      label: text.now,
      enabled: engaged,
