...
import { registerInfoBarSlot } from './info-bar-slot.js';
...
export function createController(deps = {}) {
  ...
  const request = async (action) => {
    ...
    // The info bar's model rides the same poll. Keeping the last one means a single
    // dropped request costs a stale bar, never an empty one.
    if (body.usage !== undefined && body.usage !== null) latestUsage = body.usage;
    return body;
  };

  return { read: () => latest, readUsage: () => latestUsage, sessionId: () => sessionId, request };
}
...
export function apply(ctx) {
  ...
  /**
   * Report something the operator may need to know about, without breaking the page.
   * A plugin that fails quietly is indistinguishable from one that is not installed.
   * @param {string} message - what happened.
   * @returns {void}
   */
  const warn = (message) => {
    try {
      globalThis.console?.warn?.(`[peak-valley-brake] ${message}`);
    } catch {
      /* a missing console is not worth an exception */
    }
  };
  ...
  const applySettings = () => {
    ...
    badge.setPanelLifetime(latestSettings.panelAutoHideMs);
  };
  ...
  // The info bar is separate from the badge on purpose. The badge is the brake's face and
  // can be hidden; the bar is a readout of what the account is spending, and an operator
  // who hides the character has not asked to lose the figures.
  registerInfoBarSlot(ctx, {
