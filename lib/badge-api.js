 ...
 * @param {(sessionId: string|undefined) => object|undefined} [deps.usageView] - the info bar's data model, for the session when there is one.
...
export function createBadgeRouteHandler(deps) {
  return async function handle(request) {
    ...
    // The info bar's figures are mostly not about a session at all — the model, the
    // balance, the tariff tier and the countdown are properties of the account and the
    // clock. So the usage view is built before the session question is settled, and a
    // composition with no tracked session still gets a bar rather than a blank row.
    const usage = typeof deps.usageView === 'function' ? (deps.usageView(hasSession ? sessionId : undefined) ?? null) : null;

    if (parsed.subcommand === null) {
      // A poll only reads, so it answers even when the brake has never seen a session.
      // Requiring one here would leave the info bar dark until the brake happened to hold
      // something — and on a quiet off-peak day it might never hold anything at all, which
      // is exactly when the operator most wants to see that the rate is cheap.
      if (!hasSession) {
        return json({ ok: true, kind: 'state', text: '', sessionId: null, state: null, usage }, 200);
      }
      return json(
        { ok: true, kind: 'state', text: '', sessionId, state: deps.stateForSession(sessionId) ?? null, usage },
        200,
      );
    }

    if (!hasSession) {
      // Nothing has been braked yet in this process, so there is no session to act on.
      // The badge shows its idle pose and asks again on the next poll.
      ...
      return json(
        {
          ...response.body,
          sessionId,
          state: deps.stateForSession(sessionId) ?? null,
          usage: typeof deps.usageView === 'function' ? (deps.usageView(sessionId) ?? null) : null,
        },
        response.status,
      );
    } catch (error) {
