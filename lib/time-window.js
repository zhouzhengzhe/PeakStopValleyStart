 ...
 *
 * **Bounded to roughly one window either side**, because the brake only ever
 * asks about its own next move. A caller that needs "when does the price next
 * change?" — a question whose answer can be a whole weekend away — wants
 * {@link nextTierBoundaryAfter} instead.
 *
 * @param {number} epochMs - instant to inspect.
 * @param {object} [options] - schedule options.
 * @returns {{instantMs: number, edge: 'arm'|'peak-start'|'peak-end'|'release', holdsAfter: boolean}|undefined} the next change, or `undefined` when the table never changes again.
 */
...
/**
 * The instant the billed tier next changes.
 *
 * The tariff's own question, distinct from the brake's: it asks when the *price*
 * changes, not when the guard moves. The two differ by the length of the braces,
 * and conflating them would have the info bar announce a price that is not yet in
 * force.
 *
 * Unbounded in the way it needs to be: from a Saturday afternoon the next peak is
 * on Monday, so this searches the full week rather than a window's neighbourhood.
 * Deriving it from the same table as {@link isPeakAt} keeps the countdown and the
 * tier label from ever disagreeing.
 *
 * @param {number} epochMs - instant to inspect.
 * @param {readonly PeakWindow[]} [windows] - window table override.
 * @returns {number} epoch ms of the next tier change, or `Infinity` when there is none.
 */
export function nextTierBoundaryAfter(epochMs, windows = OFFICIAL_PEAK_WINDOWS) {
  if (windows.length === 0) return Number.POSITIVE_INFINITY;
  if (!isPeakAt(epochMs, windows)) {
    // Off-peak ends when the next window opens.
    return nextWindowStart(epochMs, windows);
  }
  // Peak ends when the window containing this instant closes.
  const startMs = previousWindowStart(epochMs, windows);
  if (!Number.isFinite(startMs)) return Number.POSITIVE_INFINITY;
  return startMs + durationMinutesOfStart(startMs, windows) * MS_PER_MINUTE;
}
