...
export function createUsageRuntime(options = {}) {
  ...
  let balanceTimer;
  let lastModel = '';
  /** @type {Map<string, string>} the last model each session was seen calling. */
  const modelBySession = new Map();

  /**
   * The first argument that is a non-empty string, else `''`.
   *
   * This exists because `??` must never be used to default a model name. An empty string
   * is a *value* to `??`, and "I don't know" is exactly what a resolver returns when its
   * lookup misses — so `resolver() ?? remembered` silently keeps the miss and throws away
   * the answer. That is how this bar came to read `未选择模型` while calls were being made.
   *
   * @param {...unknown} values - candidates, best first.
   * @returns {string} the chosen name, or `''` when none is usable.
   */
  ...
  /**
   * Remember the model a call named.
   *
   * Recorded when the call is *initiated*, not when its usage arrives: the model is known
   * from the moment the request goes out, and waiting for the usage chunk would leave the
   * bar blank for the whole of the first response.
   *
   * @param {string|undefined} sessionId - the session the call belongs to.
   * @param {string} model - the model name, possibly empty.
   * @returns {void}
   */
  const rememberModel = (sessionId, model) => {
    if (typeof model !== 'string' || model === '') return;
    lastModel = model;
    if (sessionId !== undefined && sessionId !== null && String(sessionId) !== '') {
      modelBySession.set(String(sessionId), model);
    }
  };
  ...
  const record = (record) => {
    try {
      ...
      rememberModel(record.sessionId, record.model);
      scheduleSave();
    } catch (error) {
  ...
  return {
    ...
    attach(ctx) {
      ctx.on('llm/stream', async function* (hookOptions, next) {
        ...
        // `GenerateOptions.model` is required by the harness, so this is normally the model
        // itself; the resolver is only consulted if a composition somehow omits it.
        const model = firstNonEmpty(hookOptions?.model, options.modelFor?.(sessionId), lastModel);
    ...
    viewFor(sessionId) {
      const at = now();
      const remembered =
        sessionId === undefined || sessionId === null ? undefined : modelBySession.get(String(sessionId));
      return buildInfoBar({
        nowMs: at,
        // The live selection wins, because it is what the next call will use; a model this
        // session was last seen calling is the fallback; the global recall is the last resort.
        model: firstNonEmpty(options.modelFor?.(sessionId), remembered, lastModel),
