 ...
 *
 * The first version rendered at 96, which on a high-density display sampled the
 * art well below one image pixel per device pixel and read as blurry rather than
 * small. The bundled art is 320px tall, so the default sits inside that with room
 * to spare, and the whole range stays within it.
 */
export const DEFAULT_SIZE_PX = 160;

/** Smallest and largest sizes a settings control may ask for. */
export const MIN_SIZE_PX = 64;
export const MAX_SIZE_PX = 320;

/**
 * How long the toolbar survives the pointer leaving the badge.
 *
 ...
 */
export const HOVER_GRACE_MS = 600;
...
/**
 * Bring a requested size into the range the art supports.
 *
 ...
 * @returns {{position?: {x: number, y: number}, hidden?: boolean, forceShow?: boolean, sizePx?: number}} stored preferences.
 */
...
export function mountBadge(options) {
   ...
   */
  let hideTimer;
  const graceMs = Number.isFinite(options.hoverGraceMs) ? options.hoverGraceMs : HOVER_GRACE_MS;
  const revealToolbar = () => {
