...
/**
 * The toolbar's fixed width: four tabs, three gaps and 8px of padding.
 *
 ...
 */
const TAB_WIDTH = 81;
const TOOLBAR_WIDTH = 4 * TAB_WIDTH + 3 * 6 + 2 * 8;
...
export function mountBadge(options) {
  ...
  const renderPanel = (panel) => {
    ...
    // The panel's own mark, and the design fixes its colour: the header dot and the LIVE
    // pill are brand blue in every state. Deriving them from the state accent made the
    // whole header grey whenever the tariff was off-peak — which reads as "something is
    // wrong", not as "everything is cheap". The tariff's own colour is carried by the
    // row mark, where the design puts it.
