...
/* The bead: halo, shell, core, glare, at three-quarters of the design's 32px spec —
   the size the design draws it at, scaled to sit on a 160px character. The halo's
   size is the state's intensity, so quiet states cost no attention and the costly one
   is the only one that shouts. */
.pvb-bead { position: absolute; right: 4%; bottom: 8%; width: 24px; height: 24px; pointer-events: none; }
.pvb-bead-glow {
  position: absolute;
  left: 50%;
  top: 50%;
...
.pvb-badge {
  ...
  padding: 3px 8px;
  border-radius: 100px;
  /* Tinted fill *and* a border, as the design draws it: on a near-white glass panel a
     tint alone leaves the pill without an edge. */
  border: 1px solid currentColor;
  background: color-mix(in srgb, currentColor 12%, transparent);
}
.pvb-divider { height: 1px; background: rgba(255, 255, 255, 0.75); margin: 14px 0; }
.pvb-row { display: flex; align-items: baseline; justify-content: space-between; gap: 12px; }
/* The label is a slate grey by token rather than a washed-out version of the value's
   near-black: at 13px the two are the same size, so only the colour separates them,
   and a shared colour with an opacity would have made the difference mud. */
.pvb-row-label { font-size: 13px; font-weight: 500; color: var(--dsw-alias-label-secondary, #4b6175); white-space: nowrap; }
.pvb-row-value { font-size: 13px; font-weight: 500; text-align: right; }
.pvb-row-value--strong { font-weight: 600; }
.pvb-row-dot {
...
export function mountBadge(options) {
  ...
  const renderRow = (row) => {
    ...
    if (row.dotColour !== null) {
      ...
      dot.style.background = row.dotColour;
  ...
  const renderToolbar = (state) => {
    ...
    for (const button of toolbarFor(state, labels)) {
      ...
      const selected = button.action === SELECTED_TAB;
      element.setAttribute('aria-pressed', String(selected));
      element.style.color = tabColourFor(selected);
