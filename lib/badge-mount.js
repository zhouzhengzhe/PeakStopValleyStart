...
/**
 * Typography for the bubble's three kinds of row.
 *
 * The bubble says what is happening, when it stops, and how to change it. Setting
 * all three the same made it read as a paragraph; separating them lets a glance
 * land on the headline and the instruction without reading the middle.
 */
const BUBBLE_ROLE_STYLES = Object.freeze({
  title: { fontWeight: '600', fontSize: '12.5px' },
  detail: { fontSize: '11.5px', opacity: '0.72', marginTop: '2px' },
  action: { fontSize: '11.5px', marginTop: '3px', color: 'var(--dsw-alias-brand-primary, #2563eb)' },
});

/**
 * The inline style for one toolbar button.
 *
 * Weight is decided by the button's `tone`, which `toolbarFor` chooses from what the
 * operation does rather than from where it sits: reading the state is quiet,
 * releasing once is bounded and so is the solid one, and the two that either spend
 * money until the valley or reverse a decision are outlines. Disabled buttons keep
 * their place and their tooltip — the toolbar doubles as the explanation of the
 * current state, so hiding an unavailable operation would remove the explanation.
 *
 * @param {{enabled: boolean, tone: string}} button - the toolbar entry.
 * @returns {object} inline style declarations.
 */
function buttonStyleFor(button) {
  const base = {
    padding: '5px 10px',
    borderRadius: '8px',
  ...
  if (!button.enabled) {
    return { ...base, background: 'transparent', color: THEME.textMuted, opacity: '0.5' };
  }
  if (button.tone === 'primary') {
    return { ...base, background: THEME.accent, borderColor: THEME.accent, color: '#ffffff' };
  }
  if (button.tone === 'outline') {
    return { ...base, background: 'transparent', borderColor: THEME.border, color: THEME.text };
  }
  return { ...base, background: 'transparent', color: THEME.textMuted };
}

/**
 * The badge's stylesheet.
 *
 * A stylesheet rather than more inline styles, for the three things inline styles
 * cannot express: keyframes, `:hover`/`:focus-visible`, and one
 * `prefers-reduced-motion` block that switches the movement off. Everything that
 * depends on runtime numbers — position, size, the state colour — stays inline,
 * because those change per frame and per state.
 *
 * Colours here are only ever theme tokens. A misspelled custom property resolves
 * to nothing and the literal fallback wins silently, which is how the badge spent
 * its first version as a white box on a dark theme.
 *
 * The panels animate on `display` changes alone: an element coming back into the
 * layout tree restarts its animations, so showing the toolbar replays the
 * entrance without any state to keep in step with the geometry code.
 */
const BADGE_CSS = `
...
.pvb-panel { animation: pvb-pop 150ms cubic-bezier(0.2, 0.8, 0.3, 1); }
.pvb-btn {
  font: inherit;
  transition: background 120ms ease, color 120ms ease, transform 80ms ease, border-color 120ms ease;
}
.pvb-btn:not(:disabled):hover { transform: translateY(-1px); }
.pvb-btn:not(:disabled):active { transform: translateY(0); }
.pvb-btn:focus-visible {
...
.pvb-dot {
  ...
  height: 10px;
  border-radius: 50%;
  border: 2px solid var(--dsw-alias-bg-overlay, #ffffff);
...
export function mountBadge(options) {
  ...
  // The state, at a glance, when the bubble has been dismissed and the toolbar is
  // hidden. Colour alone carries it, so the shape stays out of the way.
  const dot = doc.createElement('div');
  dot.className = 'pvb-dot';
  dot.style.background = BADGE_ACCENTS.idle;
  ...
  bubble.className = 'pvb-panel pvb-bubble';
  Object.assign(bubble.style, {
    ...
    color: THEME.text,
    // The state's colour as an edge rather than a fill: the panel stays legible in
    // either theme, and the accent still reads at a glance.
    border: `1px solid ${THEME.border}`,
    borderLeft: `3px solid ${BADGE_ACCENTS.idle}`,
  ...
  toolbar.className = 'pvb-panel pvb-toolbar';
  ...
  /** The accent the last render chose; the toolbar reads it for its emphasis. */
  ...
  const render = () => {
    ...
    // One accent, three surfaces. Deriving it from the same pose decision the
    // character just took is what keeps the dot, the panel edge and the art from
    // ever describing three different moments.
  ...
  const renderBubble = (state) => {
    ...
    showBubbleLines([
      ...
      { text: content.detail, role: 'detail' },
      { text: content.action, role: 'action' },
    ]);
  };

  /**
   * Render bubble rows, dropping the empty ones.
   *
   * Three roles rather than three plain lines: the bubble carries a headline, a
   * timing, and how to change the outcome, and setting them in one weight made it
   * read as a paragraph rather than as an answer.
   *
   * @param {{text: string, role: string}[]} lines - candidate rows.
   * @returns {void}
   */
  const showBubbleLines = (lines) => {
    bubble.textContent = '';
    for (const line of lines.filter((row) => row.text !== undefined && row.text !== '')) {
      const row = doc.createElement('div');
      row.textContent = line.text;
      Object.assign(row.style, BUBBLE_ROLE_STYLES[line.role] ?? BUBBLE_ROLE_STYLES.detail);
  ...
  const renderToolbar = (state) => {
    ...
    for (const button of toolbarFor(state, labels)) {
      ...
      element.className = 'pvb-btn';
      if (button.hint !== '') element.title = button.hint;
      Object.assign(element.style, buttonStyleFor(button));
