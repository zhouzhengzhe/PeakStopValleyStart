 ...
 * @returns {{position?: {x: number, y: number}, hidden?: boolean, forceShow?: boolean, sizePx?: number, hoverDelayMs?: number, dismissedKey?: string}} stored preferences.
 */
export function readPreferences(storage) {
  ...
  try {
    ...
    return {
      ...
      ...(typeof parsed.dismissedKey === 'string' ? { dismissedKey: parsed.dismissedKey } : {}),
    };
  } catch {
    return {};
  }
...
/**
 * A signature of everything that makes the panel worth showing again.
 *
 * Deliberately excludes `updatedAtMs`, which the host moves on every poll. Including it
 * would make every poll look like news, and the panel would come straight back after each
 * dismissal — the failure this signature exists to prevent.
 *
 * `heldCount` and `lastReleaseAtMs` are in because they are the two things that actually
 * change while a window is running: another message withheld, or work released.
 *
 * @param {object|undefined} state - the state just read.
...
export function situationKey(state) {
  ...
  return [
    ...
    Math.trunc(state.heldCount ?? 0),
    state.lastReleaseAtMs ?? 0,
  ].join('|');
}
...
export function mountBadge(options) {
  ...
  /**
   * The last situation the panel was shown for; see `situationKey`.
   *
   * Seeded from the stored dismissal so that a reload does not resurrect a panel the
   * operator just closed for a situation that has not changed.
   */
  let lastSituationKey = current.dismissedKey;
  ...
  const render = () => {
    ...
    // A dismissed panel is dismissed for *that* situation, not for good. Without this the
    // first click on the character silenced the badge permanently: a message held an hour
    // later produced no panel at all, because nothing ever cleared the flag.
    const key = situationKey(state);
    if (lastSituationKey === undefined) {
      // First look after mounting. A stored dismissal was made against whatever was on
      // screen then, so it binds to what is on screen now and lapses when that changes.
      lastSituationKey = key;
    } else if (key !== lastSituationKey) {
      lastSituationKey = key;
      if (current.hidden === true) current = { ...current, hidden: false, dismissedKey: undefined };
    }
    ...
    const wasVisible = bubble.style.display === 'block';
    renderBubble(state);
    renderToolbar(state);
    place();
    // Armed on the transition into visibility, not on every render: the host polls every
    // couple of seconds, and re-arming there would restart the countdown forever and the
    // panel would never leave — which is the complaint this rule exists to answer.
    if (bubble.style.display === 'block' && !wasVisible) armPanelTimer();
  };
  ...
  /**
   * Arm the countdown that closes the panel.
   *
   * One rule for every panel, whether the operator opened it or a hold produced it. An
   * earlier version exempted the second kind on the reasoning that it explains a
   * situation and would vanish while the situation held — but a situation lasts as long
   * as the peak window does, so in practice the panel sat on screen for over an hour and
   * had to be clicked away by hand. It is a notification, not a fixture: it appears when
   * something happens, and it goes away.
   *
   * A lifetime of zero means "leave it up until dismissed", which is what an operator who
   * wants a standing readout will set.
   *
   * @returns {void}
   */
  const armPanelTimer = () => {
  ...
  /**
   * Close the panel and keep it closed for the current situation.
   *
   * @returns {void}
   */
  const dismissPanel = () => {
    current = { ...current, hidden: true, forceShow: false, dismissedKey: lastSituationKey };
    writePreferences(options.storage, current);
    render();
  };
  ...
  bubble.addEventListener('click', () => {
    // Clicking the bubble dismisses it. The state stays readable through the toolbar and
    // through the bead, so dismissal is never a dead end — and it lasts only for the
    // situation being dismissed, because the next one re-arms the panel.
    dismissPanel();
  });
