...
export const SECTION_LABELS = Object.freeze({
  ...
  panelAutoHideMs: '面板自动收起（毫秒）',
  panelAutoHideHint: `手动展开的状态面板多久后自动收起，范围 ${MIN_PANEL_MS}–${MAX_PANEL_MS}；设为 0 则一直显示，直到再次点击「状态」或点角色关闭。`,
  overridden: '已覆盖',
  reset: '恢复默认',
  resetAll: '全部恢复默认',
  section: '角色徽章',
...
export function createSettingsSection(options) {
  ...
  return function BadgeSettingsSection() {
    ...
    return React.createElement(
      ...
      row(
        'panelAutoHideMs',
        labels.panelAutoHideMs,
        labels.panelAutoHideHint,
        number('panelAutoHideMs', MIN_PANEL_MS, MAX_PANEL_MS, 500),
      ),
      anyOverridden
        ? React.createElement(
            'button',
