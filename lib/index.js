 ...
 *  - `sessionProjections` is optional, because publishing the hold for a client
 *    to display is a feature a headless composition does not have. When it is
 *    absent the brake still guards; only the badge has nothing to read.
...
export function apply(ctx, rawConfig) {
  ...
  /**
   * Read the projection registry, tolerating a composition that has none.
   * @returns {object|undefined} the registry, when resolvable.
   */
  const probeSessionProjections = () => probeService('sessionProjections');

  /**
   * Publish one state for a session.
   *
   * Every failure is reduced to a warning: a badge that cannot be updated is a
   * visibility gap, and the brake's real job — not losing work, not spending at
   * peak — must not depend on it.
   *
  ...
  const publish = (session, next) => {
    ...
    const outcome = publishHoldState(session, next, publishedStates.get(key));
    if (outcome.written) {
  ...
  /**
   * Register the projection a client subscribes to.
   *
   * Best-effort, like the command surface: a headless composition has no
   * registry, and the brake must guard there regardless — only the badge has
   * nothing to read, which is a visibility gap rather than a failure.
   *
   * Registration is an effect on this plugin's fiber, so unloading removes the
   * key and its cached cell; there is no duplicate unit to clean up on reload.
   */
  const registerProjection = () => {
    const projections = probeSessionProjections();
    if (projections === undefined || typeof projections.register !== 'function') {
      warn('no projection registry in this composition; the hold state will not be published to a client');
      return;
    }
    try {
      projections.register(holdProjectionDefinition);
    } catch (error) {
      warn(`could not register the hold projection: ${error instanceof Error ? error.message : String(error)}`);
    }
  };
  ...
  return {
    ...
    /** Whether this composition exposes a projection registry. */
    hasProjectionRegistry: () => probeSessionProjections() !== undefined,
