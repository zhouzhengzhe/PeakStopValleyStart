...
import { BADGE_ACTION_PATH, createBadgeRouteHandler } from './badge-api.js';
import { createUsageRuntime } from './usage-runtime.js';
import { modelFromAgent } from './usage-collect.js';
import { SETTINGS_NAMESPACE, normalizeBadgeSettings } from './badge-settings.js';
import { badgeSettingsBase, badgeSettingsSchema } from './badge-settings-schema.js';
 ...
 *  - `settings` is optional for the same reason: registering the badge's
 *    preferences gives them a settings page and a home in `settings.yaml`, but a
 *    composition without the settings plane must still brake.
 *
 ...
 *
 * `sessionProjections` is deliberately **not** declared. The hold state stays in
 * memory and the badge polls it from the host, because a session event of this
 * plugin's own makes the whole session log unloadable (see `hold-state.js`).
 */
export const inject = {
  commands: null,
  llm: { required: true },
  ...
  agents: null,
  settings: null,
  // Optional: the info bar's balance reading needs a credential, and a composition that
  // cannot supply one must still guard — the bar simply says the balance is unavailable.
  credentials: null,
};
...
export function apply(ctx, rawConfig) {
  ...
  /**
   * The next schedule change, as the fields the published state carries.
   *
   * The badge's panel shows "next switch", and only the host can answer it: the
   * browser has no window table and no clock the host would agree with. Computed
   * from the same table as the verdict, so a displayed time cannot disagree with the
   * guard that actually holds the request.
   *
   * @param {number} [atMs] - the instant to look forward from.
   * @returns {{nextTransitionMs: number|null, nextTransitionEdge: string|null}} the fields.
   */
  const transitionFields = (atMs = Date.now()) => {
    const upcoming = nextTransitionAfter(atMs, scheduleOptions());
    return {
      // `instantMs`, not `transitionMs`: the boundary object is the one `boundariesAround`
      // builds, and this function's own doc comment claimed otherwise for long enough that
      // the panel's "next change" row showed nothing at all.
      nextTransitionMs: upcoming?.instantMs ?? null,
      nextTransitionEdge: upcoming?.edge ?? null,
    };
  };
   ...
   *
   * A `window` override runs to the next schedule boundary, which is what bounds
   * it. A `once` override has no schedule bound of its own, so it is bounded by
   * the boundary too: an operator who asks for one release and then leaves the
   * machine must not come back to a brake that is still switched off.
   *
  ...
  const grantOverride = (kind, sessionId) => {
    ...
    // It also bounds `once`, so an operator who asks for one release and then
    // walks away does not come back to a brake that is still switched off.
    ...
    activeOverride = {
      ...
      requestedAtMs: nowMs,
      /**
       * Set when this override's release has put work back in the inbox and that
       * work has not yet been admitted. `onPreStep` reads it to let the work
       * through instead of re-parking it.
       */
  ...
  /**
   * The last state this writer published, per session.
   *
   * Kept so a no-op publish can be skipped: the brake can refuse many steps
   * inside one peak window, and writing each one would bury the session log in
   * identical records. The comparison ignores the timestamp, so the real test is
   * "nothing an operator could see changed".
   *
  ...
  /**
   * The badge settings namespace's owner scope, when the settings plane accepted
   * it. Undefined in a composition without the service.
   *
  ...
  /**
   * Read an optional cordis service, tolerating a composition that has none.
   *
   * Declaring a dependency in `inject` makes the property resolvable; it does not
   * guarantee the service is mounted. cordis signals that by throwing, so every
   * optional read is guarded rather than assumed.
   *
  ...
  /**
   * Publish one state for a session.
   *
   * In memory only. The badge reads this server-side state over its own endpoint,
   * so nothing has to enter the session log — and nothing may, because an event
   * type of this plugin's own makes the whole session unloadable (see the module
   * note in `hold-state.js`).
   *
   * @param {object} session - the session to publish against.
   * @param {object} next - the state to publish.
   * @returns {{written: boolean}} the outcome.
   */
  const publish = (session, next) => {
    if (session === undefined || session === null) return { written: false };
    const key = String(session.id ?? '');
    const outcome = nextPublishedState(next, publishedStates.get(key));
    if (outcome.written) {
      publishedStates.set(key, outcome.state);
    } else if (outcome.reason !== undefined && outcome.reason !== 'unchanged') {
      warn(`could not publish the hold state for ${key} (${outcome.reason})`);
    }
    return outcome;
  };

  /**
   * The last state this plugin published for a session.
   *
   * Keyed by session id, which is what `publish` writes — but an id is read from
   * the agent in most of this file, and the two are only equal by convention.
   * Consulting both keys costs one map lookup and removes a class of silent
   * `null` where a carried-forward field should be.
   *
   * @param {object} agent - the agent owning the session.
   * @returns {object|undefined} the last published state, when there is one.
   */
  const lastPublishedFor = (agent) => {
    const sessionId = agent?.session?.id;
    if (sessionId !== undefined && publishedStates.has(String(sessionId))) {
      return publishedStates.get(String(sessionId));
    }
    return publishedStates.get(String(agent?.id ?? ''));
  };
  ...
  /**
   * Describe a message's shape for a failure record, without its text.
   *
   * When delivery fails, the first question is whether the message was
   * well-formed — a missing id, the wrong role, a frozen-object problem. The
   * shape answers that; the content is the operator's private prompt and has no
   * business in a log file.
   *
   * @param {object} message - the message that failed to deliver.
   * @returns {object} field names, types, and content block kinds.
   */
  const describeMessageShape = (message) => ({
    keys: Object.keys(message ?? {}).sort(),
    idType: typeof message?.id,
    idLength: typeof message?.id === 'string' ? message.id.length : 0,
    role: message?.role,
    frozen: Object.isFrozen(message),
    contentBlocks: Array.isArray(message?.content) ? message.content.map((block) => block?.type ?? typeof block) : null,
  ...
  const deliverHeldOnce = async (state, agent) => {
    ...
    let delivered = 0;
    /** The first failure, kept whole so the reason can be reported and logged. */
    let failure;
    for (const [index, message] of restored.entries()) {
      const via = index === 0 ? 'send' : 'inbox.append';
      try {
        // Arm the override before the delivery, so the admission that follows it
        // synchronously is admitted rather than re-parked. See `onPreStep`.
        if (override?.kind === 'once') override.awaitingAdmission = true;
        if (index === 0) agent.send(message, 'next-turn', true);
        else agent.inbox.append('next-turn', message);
        delivered += 1;
      } catch (error) {
        if (override?.kind === 'once') override.awaitingAdmission = false;
        failure ??= { via, message, error };
        warn(
          `re-delivery of message ${String(message.id)} via ${via} failed: ` +
            `${error instanceof Error ? `${error.name}: ${error.message}` : String(error)}`,
        );
      }
    }

    // Record the attempt whatever happened. A failed delivery used to leave the
    // operator with nothing but a silent absence and a log line they cannot see,
    // which is how "the command said it was releasing and then nothing happened"
    // became an undiagnosable report.
    const outcome = await ledger.appendDeliveryOutcome({
      sessionId: key,
      outcome: delivered === restored.length ? 'delivered' : 'failed',
      attempted: restored.length,
      delivered,
      ...(failure === undefined
        ? {}
        : {
            ...
            messageId: String(failure.message.id),
            messageShape: describeMessageShape(failure.message),
          }),
    });
    if (!outcome.ok) warn(`could not record the delivery outcome (${outcome.code})`);
    ...
    if (delivered === restored.length) {
      ...
      // Publish the release so a client stops showing the hold. `override` is
      // read before the override is spent at admission, so the badge can say a
      // release happened by operator choice rather than by schedule.
      publish(
        agent.session,
        releaseStateFor({
          phase: verdict.phase.state,
          releaseReason: override !== undefined ? 'override' : 'schedule',
  ...
  const releaseAll = async () => {
    ...
    // A one-shot override is not spent here. Spending it at delivery is what
    // caused the released message to be re-parked in the same instant, because
    // the driver claims the message immediately and this guard then found the
    // override already consumed. It is spent at admission instead — see
    // `onPreStep` — so `once` means "this work passes", not "this work is moved".
    armSharedTimer();
  };
  ...
  const onPreStep = async (payload, next) => {
    ...
    const arriving = Array.isArray(messages) ? messages.length : 0;
    const override = activeOverride;

    // A one-shot override is spent where its work is *admitted*, not where it is
    // released.
    //
    // Spending it at release was a real defect. Releasing puts the message back
    // in the inbox, the driver claims it immediately, and the guard then found
    // its own override already consumed while peak was still in force — so it
    // parked the message again in the same instant. The operator saw "releasing
    // now" and then nothing, with the message back in the ledger.
    //
    // A one-shot release has to mean "this work passes", not "this work is
    // moved". So: spend it when work actually arrives, and leave it unspent when
    // the wake carries nothing to admit — otherwise a stray empty wake would burn
    // the operator's override and their work would stay parked.
    if (override?.kind === 'once' && override.awaitingAdmission === true && arriving > 0) {
      activeOverride = undefined;
      warn('one-shot manual override spent on the work it released');
      return next();
    }
    ...
    if (held.length > 0) {
      ...
      // Publish the hold for a client to display. This is the durable channel:
      // unlike the injected receipt above, a session event is not discarded when
      // a step is rejected, and a client subscribes to it rather than hoping a
      // notice renders.
      publish(
        agent.session,
        holdStateFor({
          ...
          releaseAtMs: verdict.phase.releaseMs,
          override: activeOverride,
          lastReleaseReason: lastPublishedFor(agent)?.lastReleaseReason,
          lastReleaseAtMs: lastPublishedFor(agent)?.lastReleaseAtMs,
          nowMs,
          ...transitionFields(nowMs),
        }),
      );
   ...
   */
  const probeCommands = () => probeService('commands');
  ...
  const commandHandler = async (agent, rawInput) => {
    ...
    if (subcommand === 'status') {
      ...
      // Surface the most recent failed re-delivery for this session. Without this
      // the operator sees a release that reports success and a message that never
      // arrives, with no way to tell why.
      const attempts = await ledger.readDeliveryOutcomes(40);
      const lastFailure = attempts.filter((record) => record.sessionId === String(agent.id) && record.outcome === 'failed').at(-1);
      if (lastFailure !== undefined) {
        lines.push(
          render(text, `${T}.statusLastDeliveryFailed`, {
            atUtc: new Date(lastFailure.atMs).toISOString().replace('T', ' ').slice(0, 16),
  ...
  /**
   * The id a session is addressed by on the wire.
   *
   * `stateFor` keys by agent id while `publish` keys by session id; the two agree
   * by convention rather than by construction, so one function decides and every
   * caller uses it.
   *
   * @param {object} agent - the agent owning the session.
   * @returns {string} the session id, or an empty string when there is none.
   */
  const sessionIdOf = (agent) => String(agent?.session?.id ?? agent?.id ?? '');

  /**
   * Find the live agent behind a session id.
   *
   * The `agents` service is the authority when it exists, but it is an optional
   * dependency: in a composition without it every button would 404 even though
   * this plugin is already holding the very agent objects it needs. So the held
   * states are consulted too, and the route keeps working either way.
   *
   * @param {string} sessionId - the session to resolve.
   * @returns {object|undefined} the agent, when one is live or held.
   */
  const agentForSession = (sessionId) => {
    const wanted = String(sessionId);
    const agents = probeService('agents');
    if (agents !== undefined && typeof agents.get === 'function') {
      try {
        const live = agents.get(wanted);
        if (live !== undefined && live !== null) return live;
      } catch {
        /* fall through to the agents this plugin already holds */
      }
    }
    for (const state of states.values()) {
      const agent = state.agent;
      if (String(agent?.id ?? '') === wanted || String(agent?.session?.id ?? '') === wanted) {
        return agent;
      }
    }
    return undefined;
  ...
  /**
   * The session a badge should describe when its request names none.
   *
   * The browser cannot answer this: this harness provides no session registry to
   * client plugins, so the host picks the most recently created session it is
   * still holding. That is the session the operator is looking at in every
   * ordinary single-window case, and the badge hands the id straight back on its
   * next request — so this decision is made once per page load, not per poll.
   *
   * @returns {string|undefined} the session id, or undefined when nothing is tracked yet.
   */
  const currentSessionId = () => {
    for (const state of [...states.values()].reverse()) {
      const id = sessionIdOf(state.agent);
      if (id !== '') return id;
    }
    return [...publishedStates.keys()].at(-1);
  };

  /**
   * Read the last state published for a session, tolerating either id.
   *
   * @param {string} sessionId - the session to read.
   * @returns {object|undefined} the published state, when there is one.
   */
  const publishedStateFor = (sessionId) => {
    const direct = publishedStates.get(String(sessionId));
    if (direct !== undefined) return direct;
    const agent = agentForSession(sessionId);
    return agent === undefined ? undefined : publishedStates.get(sessionIdOf(agent));
  };

  /**
   * The state to hand a client right now, whether or not one was ever published.
   *
   * `publishedStates` only holds what the brake has already announced, and the brake
   * announces on a step — so a session that has not yet had a message held has no
   * published state at all, and an earlier version of this route answered `null` for it.
   * The badge then had nothing to draw and the status panel came up empty, which is
   * indistinguishable from the button being broken.
   *
   * The schedule, however, is always answerable: it is a pure function of the clock and
   * the window table. So the live verdict is composed on top of whatever was published,
   * and the only parts kept from the published record are the ones that are genuinely
   * historical — when the last release happened and why.
   *
   * @param {string} sessionId - the session to describe.
   * @returns {object} a complete state.
   */
  const liveStateFor = (sessionId) => {
    const agent = agentForSession(sessionId);
    const verdict = verdictNow();
    return composeLiveState(publishedStateFor(sessionId), {
      hold: verdict.hold,
      reason: verdict.reason,
      phase: verdict.phase.state,
      releaseAtMs: verdict.phase.releaseMs,
      heldCount: agent === undefined ? 0 : (states.get(String(agent.id))?.held.size ?? 0),
      override: isOverrideLive() ? activeOverride : undefined,
      nowMs: Date.now(),
      ...transitionFields(),
    });
  };
  ...
  /**
   * Register the endpoint the badge's buttons call.
   *
   * The path is claimed under `/api` because that prefix has exactly one owner in
   * this harness, and that owner performs browser authentication and Host/Origin
   * validation before dispatching to registered exact paths. A route anywhere else
   * would stand outside that fence.
   *
   * Every action runs the same `commandHandler` the `/peak-valley` command runs,
   * so a button and the command line cannot drift apart — there is one
   * implementation of each operation.
   *
   * Best-effort: a `file://` deployment or a headless composition has no
   * connection service, and the brake must guard regardless. Only the buttons go
   * dark.
   */
  const registerBadgeRoute = () => {
    const connection = probeService('connection');
    if (connection === undefined || typeof connection.fetch?.register !== 'function') {
      warn('no connection fetch registry in this composition; the badge buttons will be unavailable');
      return;
    }
    if (typeof commandHandler !== 'function') return;
    try {
      connection.fetch.register({
        path: BADGE_ACTION_PATH,
        methods: ['POST'],
        requestBody: 'buffered',
        fetch: createBadgeRouteHandler({
          agentForSession,
          resolveSessionId: currentSessionId,
          stateForSession: liveStateFor,
          runCommand: (agent, subcommand) => commandHandler(agent, subcommand),
          usageView: (sessionId) => {
            // Building the bar's figures must never be able to fail a poll: the badge is
            // answered by the same request, so a throw here would take the badge down with
            // it. Null means "no figures yet", which the bar renders as absent.
            try {
              return usage?.viewFor(sessionId) ?? null;
            } catch (error) {
              warn(`info bar: could not build the view: ${error instanceof Error ? error.message : String(error)}`);
              return null;
            }
          },
        }),
      ...
      warn(`could not register the badge action endpoint: ${error instanceof Error ? error.message : String(error)}`);
    }
  };

  /**
   * The info bar's host side: a spend ledger, a balance cache, and the model-stream tap.
   *
   * Built before the route so the route can read it. Every failure here is contained:
   * the bar is a convenience, the brake is the product. A composition whose credential
   * service, home directory or stream hook misbehaves must still withhold messages at
   * peak, so this whole block is allowed to fail — and says so when it does, because a
   * silently missing bar is indistinguishable from a broken one.
   */
  let usage;
  try {
    usage = createUsageRuntime({
      report: (message) => warn(`info bar: ${message}`),
      resolveCredential: async (name) => {
        const credentials = probeService('credentials');
        ...
        // The service may hand back a bare string or a record holding one; both shapes are
        // accepted so a change on the harness side degrades to "no balance" rather than a
        // crash. The value is used here and nowhere else — never logged, never sent out.
        if (typeof resolved === 'string') return resolved;
        if (resolved !== null && typeof resolved === 'object') {
          for (const key of ['value', 'token', 'secret', 'apiKey']) {
            if (typeof resolved[key] === 'string') return resolved[key];
          }
        }
        return undefined;
      },
      sessionIdFor: (hookOptions) => {
        const named = hookOptions?.sessionId;
        if (typeof named === 'string' && named !== '') return named;
        return currentSessionId();
      },
      modelFor: (sessionId) =>
        modelFromAgent(sessionId === undefined ? undefined : agentForSession(sessionId)),
    });
    void usage.load();
    usage.attach(ctx);
  } catch (error) {
    warn(
      `info bar: could not start the usage runtime, the bar will be absent: ${error instanceof Error ? error.message : String(error)}`,
    );
    usage = undefined;
  }
  ...
  /**
   * Hand the badge's preferences to the harness settings plane.
   *
   * Registering a namespace is what puts the values in `settings.yaml`, gives them
   * the settings page's "overridden / restore default" behaviour, and lets the
   * browser read them through `settingsScope` instead of this plugin inventing a
   * transport for them.
   *
   * Fail-soft in two directions. A composition with no settings service simply has
   * no settings page and the badge falls back to its own defaults — the brake must
   * never depend on a cosmetic preference. And a stored section that the schema
   * rejects (a hand-edited `settings.yaml`, say) must not take the brake down with
   * it, so the registration is guarded rather than trusted.
   */
  const registerSettings = () => {
    const settings = probeService('settings');
    if (settings === undefined || typeof settings.register !== 'function') {
      warn('no settings service in this composition; the badge has no settings page and will use its defaults');
      return;
    }
    try {
      settingsScope = settings.register(SETTINGS_NAMESPACE, badgeSettingsSchema, { base: badgeSettingsBase });
    } catch (error) {
      warn(`could not register the badge settings: ${error instanceof Error ? error.message : String(error)}`);
    }
  };
  ...
  return {
    ...
    /** The live override, for tests and operator consoles. */
    debugOverride: () => activeOverride,
    /** The last state published for a session, for tests and operator consoles. */
    publishedState: (sessionId) => publishedStates.get(String(sessionId)),
    /** The state a client would be handed right now, published or not. */
    liveState: (sessionId) => liveStateFor(sessionId),
    /** The badge settings as the host resolved them, for tests and operator consoles. */
    badgeSettings: () => normalizeBadgeSettings(settingsScope?.get()),
    /** Whether this composition accepted the badge's settings namespace. */
    hasSettingsPlane: () => settingsScope !== undefined,
    ...
    /** The info bar's data model, for tests and operator consoles. */
    usageView: (sessionId) => usage?.viewFor(sessionId) ?? null,
    /** The spend ledger as this instance holds it, for tests and operator consoles. */
    usageLedger: () => usage?.ledgerState(),
    /** Whether this composition started the info bar's runtime at all. */
    hasUsageRuntime: () => usage !== undefined,
  };
}
